 ...
 *  聊天设置
 */
public class ChatSetFragment extends BaseFragment implements View.OnClickListener{
    ...
    public int getLayoutResId() {
        return R.layout.fragment_msg_chat_set;
    }
...
//                startActivity(intent);
            ...
            case R.id.widget5:
                intent = new Intent(getActivity(), ZFrameActivity.class);
                intent.putExtra(ZFrameActivity.CLASS_NAME, FileFragment.class.getName());
                startActivity(intent);
                break;
        }
    }
    private void  initDialog1(String title,String des){
        CommonDialog dialog = new CommonDialog(getActivity());
        dialog.setTitle(title);
        View view=getLayoutInflater(R.layout.dialog_text);
        TextView desTv=(TextView)view.findViewById(R.id.edit);
        desTv.setText(des);
        dialog.setMiddleView(view);
        //dialog.setDisplayMiddleEnable(false);
        dialog.setOnClickCancelListener("取 消", new View.OnClickListener() {
