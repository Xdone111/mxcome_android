...
import com.fenazola.mxcome.entry.DesignerEntry;
...
public class DesignerFragment extends BaseFragment implements View.OnClickListener {
    ...
    private CommonAdapter<DesignerEntry> adapter;
    private RadioGroup radioGroup;
    private FrameLayout classify_layout;
    private ImageView image1, image2, image3, image4, image5;
    private List<DesignerEntry> datas;
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        if (args.containsKey(Constant.key1)) {
            skill = args.getString(Constant.key1);
        }
        adapter = new CommonAdapter<DesignerEntry>(getActivity(), R.layout.listitem_designer_circle) {
            @Override
            protected void convert(ViewHolder viewHolder, DesignerEntry item, int position) {
                ...
                order_no.setText("接单量 " + item.getJdl());
                distance.setText(item.getDistance());
                score.setRating(Float.parseFloat(item.getScore()));

                Glide.with(getActivity()).load(item.getPic()).error(R.mipmap.design_house).into(logo);
        ...
        mScrollPicker.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                DesignerEntry entry = adapter.getItem(position);
        ...
        mScrollPicker.setOnItemSelectedListener(new ScrollPicker.OnItemSelectedListener() {
            ...
            public void onItemSelectedSchedu(ScrollPicker picker, int position) {
                DesignerEntry designer = datas.get(position);
            ...
            public void onItemSelectedTalk(ScrollPicker picker, int position) {
                try {
                    DesignerEntry worker = datas.get(position);
    ...
    public void queryData() {
        ...
        NetWorkUtils.post(getActivity(), "appApi/queryDesigner.do", map, new NetWorkUtils.IListener() {
            @Override
            public void onSuccess(String result, JSONObject resObj) {
                String res = resObj.optString("res");
                ...
                if (pageIndex == 1) {
                    datas = GsonUtils.getListFromJSON(res, new TypeToken<ArrayList<DesignerEntry>>() {
                    }.getType());
    ...
    public void setTopBar() {
        ...
        imgView.setImageResource(R.mipmap.designer_title);
