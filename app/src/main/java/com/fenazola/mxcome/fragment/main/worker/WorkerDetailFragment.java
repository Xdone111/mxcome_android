...
public class WorkerDetailFragment extends BaseFragment implements View.OnClickListener {
    ...
    private TagLayout service_taglayout; //服务内容
    ...
    private FrameLayout project_show; //项目展示
    ...
    public void initView() {
        ...
        service_taglayout = (TagLayout) findViewById(R.id.service_taglayout); //服务内容
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        Glide.with(getActivity()).load(Constant.baseUrl + entry.getBase().getPic()).error(R.mipmap.me_photo).into(img_photo);
        ...
        CommonAdapter<String> serviceAdapter = new CommonAdapter<String>(getContext(), R.layout.tagitem_service) {
            @Override
        ...
        service_taglayout.setAdapter(serviceAdapter);
        List<String> datas = new ArrayList<>();
        if (entry.getBase().getSkill_style() != null) {
            List<String> skillStyle = entry.getBase().getSkill_style();
            for (int i = 0; i < skillStyle.size(); i++) {
                String string = Utils.numberToString(Integer.parseInt(skillStyle.get(i)));
                datas.add(string);
            }
            serviceAdapter.addAll(datas);
        }
        ...
        now_schedu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dialog.setMiddleBgRes(R.mipmap.order_worker_bg);
    ...
    private void changeItemH() {
        RelativeLayout.LayoutParams linearParams = (RelativeLayout.LayoutParams) top_bg.getLayoutParams();
        linearParams.height = (int) ((float) width * 0.453);
        top_bg.setLayoutParams(linearParams);
    }
    ...
    public void refreshUI() {
        if (!TextUtils.isEmpty(entry.getComment())) {
            ...
            if (!TextUtils.isEmpty(entry.getTotal_hp()) && !TextUtils.isEmpty(entry.getTotal_zp()) && TextUtils.isEmpty(commentEntry.getCp())) {
            ...
            if (commentEntry != null) {
                ...
                if (!TextUtils.isEmpty(commentEntry.getPic_url())) {
                    LogUtils.i("photo----------", commentEntry.getPic_url());
    ...
    public void setTopBar() {
        ...
        collect.setImageResource(R.mipmap.collect_white);
