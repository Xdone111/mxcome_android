...
import com.fenazola.mxcome.utils.BaseTools;
 ...
 */
public class CurtainView1 extends RelativeLayout implements OnTouchListener{
	...
	public CurtainView1(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public CurtainView1(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	/** 初始化 */
	private void init(Context context) {
		...
		final View view = LayoutInflater.from(mContext).inflate(R.layout.curtain1, null);
		img_curtain_ad = (RelativeLayout) view.findViewById(R.id.img_curtain_ad);
		img_curtain_rope = (RelativeLayout)view.findViewById(R.id.img_curtain_rope);
		addView(view);
		img_curtain_ad.post(new Runnable() {
			...
			public void run() {
				...
				CurtainView1.this.scrollTo(0, curtainHeigh);
	...
	/**
	 * TODO 点击绳索开关，会展开关闭 要跳转到 红包页面
	 * 在onToch中使用这个中的方法来当点击事件，避免了点击时候响应onTouch的衔接不完美的影响
	 *
	 */
	public void onRopeClick(){
		if(isOpen){
			CurtainView1.this.startMoveAnim(0, curtainHeigh, upDuration);
		}else{
			CurtainView1.this.startMoveAnim(curtainHeigh,-curtainHeigh, downDuration);
		}
		isOpen = !isOpen;
	}
}
